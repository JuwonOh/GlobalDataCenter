plot.data <- df.all %>% filter(str_detect(ngram, paste0(word.list, collapse = "|")))
...
p <- plot.data %>%
  ...
  labs(title=paste0("Bigram Words Related with ", paste0(word.list, collapse = ", "), " in ", input, " Articles"), subtitle = subtitle, caption = "Copyright: SNU IIS Global Data Center")

pdf(file=paste0(file.name, "_", word.list[[i]], "_all.pdf"), family="sans", width = 12, height = 7)
print(p)
dev.off()
png(file=paste0(file.name, "_", word.list[[i]], "_all.png"), family="sans", width = 205, height = 205, units='mm', res = 150)
print(p)
dev.off()
